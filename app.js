...
6. 提供一個針對性的「補救教學建議」（remediation），面向教師，說明具體的課堂引導手法。請依照下方「臺灣數學教育研究推薦的四大補救教學策略」，根據所診斷出的迷思概念類型，選擇最合適的策略撰寫建議：

【四大補救教學策略參考】
▶ 策略一：具體-半具體-抽象（CRA）教學順序
  適用迷思：分數等分概念缺失、離散量分數混淆、位值概念不穩、乘法意義理解薄弱等需要從直觀操作重建概念的情形。
  操作要點：① 具體階段：讓學生操作實體教具（如摺紙、實物分裝）體驗概念核心。② 半具體階段：在紙上畫圖（圓形塗色、線段分組）呈現抽象關係。③ 抽象階段：最後才連結算式符號，引導學生說出符號的意義。

▶ 策略二：認知衝突策略（Cognitive Conflict）
  適用迷思：小數長度大小迷思、整數思維干擾分數比較（Whole Number Bias）、「越乘越大」遷移錯誤等需打破學生既有直覺的情形。
  操作要點：① 引導學生先做出錯誤預測。② 呈現與預測矛盾的情境（如用錢幣或定位板反例）。③ 引導學生反思、主動重建正確認知架構。

▶ 策略三：圖式架構與線段圖（Schema-Based Instruction）
  適用迷思：比較型文字題關鍵字迷思（基準量未知）、基準量與比較量混淆、兩步驟問題結構不清等語意理解錯誤。
  操作要點：① 語意分析：找出題目三要素（基準量、比較量、差異量）。② 繪製線段圖：將文字關係轉為視覺線段，標示已知與未知。③ 根據圖示列式，避免直覺抓關鍵字。

▶ 策略四：數線與定位板整合運用
  適用迷思：小數點定位錯誤、小數大小比較、分數與小數互換混淆、直式計算位值未對齊等。
  操作要點：① 數線比大小：在 0 到 1 的數線上標示分數與小數，以點的相對位置釐清大小。② 定位板運算：強制要求學生在畫有個位、十分位、百分位的定位板上書寫，以物理線條限制對齊行為。
...
JSON 格式要求：
{
  ...
  "remediation": "【給教師的補救教學建議】請說明：① 診斷出的迷思類型屬於哪種認知成因（如整數思維干擾、等分概念缺失、語意理解不足等）。② 建議採用哪一種教學策略（CRA具體-半具體-抽象 / 認知衝突策略 / 線段圖教學 / 數線與定位板）及其原因。③ 具體的課堂操作步驟或引導問句（至少提供2-3個可直接使用的教學活動或問句）。"
}`;
