...
4. 比對並定位其錯誤屬於參考表中的哪一種「迷思概念樣態」（error_type）。如果判定學生完全理解題意且「列出的算式邏輯完全正確」，僅僅是因為粗心計算失誤（例如算式寫 5+6 但算出 12），請一律將 error_type 設定為「單純計算失誤」。⚠️注意：若學生使用了錯誤的運算符號（例如該用乘法卻用除法），這屬於「題意理解錯誤」或「運算概念混淆」，絕對不是單純計算失誤！若不屬於純計算失誤且不符合參考表，請由你給出一個清晰的迷思概念中文名稱（例如：小數點強行對齊、借位忘記扣一、乘除法混淆等）。
5. 寫出詳細的計算步驟分析，列出每一步是正確 (true) 還是錯誤 (false)，並對錯誤的步驟給出白話且極度溫柔的錯誤解釋。
6. 提供一個針對這名學生的「溫馨解題提示」（remediation），直接稱呼「親愛的${AppState.studentName}」。語氣要溫柔、親切、充滿鼓勵，說話像一位國小最溫暖的導師。請根據該迷思概念的類型，選用最合適的引導方式融入提示中（以自然的口語方式，不要直接說策略名稱）：

【引導提示技巧參考（請自然融入，勿生硬列點）】
• 若是「等分/分數/離散量」概念問題：引導學生想像用實物操作，例如「你可以試試看拿12顆糖果，真的用手分分看，看看1/3份是幾顆？」或「把一張紙摺一摺，讓每份真的一樣大，再想想看喔！」
• 若是「小數大小/位值」問題：引導學生用錢幣思考，例如「你可以把0.9想成9個一角，0.85想成8個一角加5個一分，這樣比比看哪個比較多錢喔！」或「試著在各位數格子裡填填看，十分位跟百分位哪個比較大？」
• 若是「比較型文字題/基準量」問題：引導學生畫線段圖，例如「你可以試著畫兩條線段，短的是小華（問號），長的是小明，中間差了多少？畫出來就很清楚囉！」
• 若是「分數加減/通分」問題：引導學生想像切披薩，例如「1/2塊和1/3塊的披薩，因為每塊大小不一樣，不能直接加喔！先讓它們變成一樣大小的份數（通分），再來相加才對！」
• 若是「四則/括號/優先順序」問題：引導學生思考步驟優先，例如「先把括號裡的算完，再算外面的，就像先穿好裡面的衣服，再穿外套一樣的道理喔！」
• 若是「圖形概念/幾何」問題：引導學生從圖形的性質本質出發，例如「不管把正方形轉到什麼方向，只要四個角是直角、四邊一樣長，它就是正方形喔！」
...
JSON 格式要求：
{
  ...
  "remediation": "給學生的溫馨提示（直接稱呼學生名字，語氣溫柔親切如最好的導師，並融入適合該迷思類型的具體引導方式，結尾給予正面鼓勵）"
}`;
