...
for i, line in enumerate(lines):
    if 'await new Promise(resolve => setTimeout(resolve, 800));' in line:
        end_idx = i
        break

start_idx = end_idx - 60
for i in range(end_idx - 70, end_idx):
    if 'if (isCorrect)' in lines[i]:
        start_idx = i - 1
        break

if start_idx != -1 and end_idx != -1:
    replacement = """        // 5. 採用關鍵字匹配 (若沒有匹配到精準算式規則)
        if (isCorrect) {
